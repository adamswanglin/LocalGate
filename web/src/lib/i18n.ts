...
const MESSAGES: Record<string, { zh: string; en: string }> = {
  ...
  'nav.modelEntries': { zh: '模型入口', en: 'Model Entries' },
  'nav.modelEntriesDesc': { zh: 'API 暴露入口', en: 'API entry points' },
  ...
  'sources.modelsLabel': { zh: '模型与价格（元/百万 token；模型入口绑定上游后从这些模型中选择）', en: 'Models & prices (CNY/M tokens; model entries pick from these when binding)' },
  ...
  // ── 模型入口 Model Entries ──
  'modelEntries.title': { zh: '模型入口', en: 'Model Entries' },
  'modelEntries.subtitle': { zh: '一个对外模型 ID 可配置多个 API 类型，每个类型可绑定多个上游并人工切换', en: 'One exposed model ID can serve multiple API types; each type binds multiple upstreams you can switch manually' },
  'modelEntries.add': { zh: '新增入口', en: 'New Entry' },
  'modelEntries.statTotal': { zh: '总入口数', en: 'Total Entries' },
  'modelEntries.statEnabled': { zh: '已启用', en: 'Enabled' },
  'modelEntries.statProtocols': { zh: '协议类型', en: 'Protocol Types' },
  'modelEntries.tabTokens': { zh: 'AccessToken', en: 'AccessToken' },
  'modelEntries.tabModel': { zh: 'Model', en: 'Model' },
  'modelEntries.newModel': { zh: '新增模型', en: 'New Model' },
  'modelEntries.editGroup': { zh: '编辑', en: 'Edit' },
  'modelEntries.deleteGroup': { zh: '删除模型', en: 'Delete model' },
  'modelEntries.confirmDeleteGroup': { zh: '确认删除该模型？将移除其所有 API 类型与上游绑定。', en: 'Delete this model? All its API types and upstream bindings will be removed.' },
  'modelEntries.colModel': { zh: '对外模型名', en: 'Exposed Model' },
  'modelEntries.colUpstream': { zh: '当前上游', en: 'Active Upstream' },
  'modelEntries.colPrice': { zh: '价格 (元/百万)', en: 'Price (CNY/M)' },
  'modelEntries.colStatus': { zh: '状态', en: 'Status' },
  'modelEntries.colActions': { zh: '操作', en: 'Actions' },
  'modelEntries.empty': { zh: '暂无模型', en: 'No models yet' },
  'modelEntries.emptyDesc': { zh: '点击「新增模型」来添加一个对外模型 ID', en: 'Click "New Model" to add an exposed model ID' },
  'modelEntries.noBinding': { zh: '无绑定', en: 'No binding' },
  'modelEntries.switchActive': { zh: '切换当前生效的上游', en: 'Switch active upstream' },
  'modelEntries.modalEdit': { zh: '编辑模型', en: 'Edit Model' },
  'modelEntries.modalCreate': { zh: '新增模型', en: 'New Model' },
  'modelEntries.fieldName': { zh: '名称（可选）', en: 'Name (optional)' },
  'modelEntries.placeholderName': { zh: '如 对外聊天入口', en: 'e.g. Chat Gateway' },
  'modelEntries.fieldModel': { zh: '对外暴露的模型 ID（客户端请求 body.model 用这个）', en: 'Exposed model ID (used as body.model by clients)' },
  'modelEntries.placeholderModel': { zh: '如 gpt-4', en: 'e.g. gpt-4' },
  'modelEntries.fieldApiTypes': { zh: 'API 类型（每个类型可配多个上游）', en: 'API types (each can bind multiple upstreams)' },
  'modelEntries.fieldApiTypesHint': { zh: '默认展示全部三类；未配置上游的类型不会保存。', en: 'All three types are shown by default; types with no upstream configured are not saved.' },
  'modelEntries.typeUnconfigured': { zh: '未配置（留空不保存）', en: 'Not configured (left empty — not saved)' },
  'modelEntries.legacyNoName': { zh: '（旧入口·未命名，建议删除或编辑补全）', en: '(legacy entry · unnamed — delete or edit to set a model ID)' },
  'modelEntries.fieldProtocol': { zh: '入站 API 类型（上游源需已配置对应协议地址）', en: 'Inbound API type (the upstream source must have a matching protocol endpoint)' },
  'modelEntries.bindingsLabel': { zh: '上游绑定（选择上游源 → 该源已配置的模型；单选生效）', en: 'Upstream bindings (pick source → its models; one active)' },
  'modelEntries.bindingsWarn': { zh: '没有已配置该协议地址的上游源，请先在「上游源」添加对应协议地址并配置模型。', en: 'No upstream source has this protocol endpoint configured — add one in "Sources" first.' },
  'modelEntries.bindingsEmpty': { zh: '尚未添加上游，请点击下方按钮。', en: 'No upstreams yet — click below to add.' },
  'modelEntries.bindingActive': { zh: '生效', en: 'Active' },
  'modelEntries.placeholderSelectSource': { zh: '-- 选择上游源 --', en: '-- Select source --' },
  'modelEntries.placeholderSelectModel': { zh: '-- 选择该上游的模型 --', en: '-- Select model --' },
  'modelEntries.placeholderSelectSourceFirst': { zh: '请先选择上游源', en: 'Select a source first' },
  'modelEntries.addBinding': { zh: '添加上游', en: 'Add Upstream' },
  'modelEntries.alertForm': { zh: '请填写对外模型 ID', en: 'Please fill in the exposed model ID' },
  'modelEntries.alertBindings': { zh: '请至少添加一个上游绑定', en: 'Add at least one upstream binding' },
  'modelEntries.alertBindingRows': { zh: '每个绑定都需要选择上游源和模型', en: 'Each binding needs a source and model' },
  'modelEntries.confirmDelete': { zh: '确认删除该模型入口？', en: 'Delete this model entry?' },
  'modelEntries.alertSwitchFailed': { zh: '切换失败', en: 'Switch failed' },
  ...
  'detail.metaEntry': { zh: '入口', en: 'Entry' },
  ...
  'stats.subtitle': { zh: '上游源 / 模型入口 的 Token 与调用量统计', en: 'Token & call volume stats for sources / model entries' },
  'stats.groups.day': { zh: '按天', en: 'Daily' },
  'stats.groups.month': { zh: '按月', en: 'Monthly' },
  'stats.groups.source': { zh: '按上游源', en: 'By Source' },
  'stats.groups.entry': { zh: '按入口', en: 'By Entry' },
  ...
  'stats.filterEntry': { zh: '入口', en: 'Entry' },
  'stats.filterAllEntries': { zh: '全部入口', en: 'All Entries' },
  ...
  'stats.stackDim3': { zh: '入口·模型用量（按模型堆叠）', en: 'Entry × model usage (stacked by model)' },
  ...
  'update.ready': { zh: '更新已就绪，重启以应用', en: 'Update ready, restart to apply' },
  'update.download': { zh: '下载更新', en: 'Download' },
  'update.install': { zh: '立即重启', en: 'Restart now' },
  'update.dismiss': { zh: '稍后', en: 'Later' },
  'update.checking': { zh: '检查更新中…', en: 'Checking for updates…' },
  'update.latest': { zh: '已是最新版本', en: 'You are on the latest version' },
  'update.failed': { zh: '检查更新失败', en: 'Update check failed' },
};
