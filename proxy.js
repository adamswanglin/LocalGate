...
import { PROTOCOLS } from '../lib/protocol.js';
...
const NULL_PRICING = { inputPrice: null, cachedInputPrice: null, outputPrice: null };
...
export function getDispatcher(proxyUrl) {
    ...
    if (currentDispatcher && p === currentProxyUrl)
...
async function checkAuth(headers) {
    ...
    if (!enabledTokens.length)
        return { ok: true }; // 未配置 token：空 key 放行
...
/* ---------------- 入口 / 绑定 / 上游解析 ---------------- */
async function findChannel(exposedModel, protocol) {
    const rows = await db
        .select()
 ...
 */
async function resolveActiveBinding(channel) {
...
function buildUpstreamUrl(baseUrl, protocol) {
    const base = baseUrl.replace(/\/+$/, '');
    return base + PROTOCOLS[protocol].upstreamSuffix;
}
function buildUpstreamHeaders(protocol, apiKey, incoming) {
    const h = new Headers();
...
function computeCost(norm, prices) {
    const costOf = (tokens, price) => price != null && price > 0 ? (tokens / 1_000_000) * price : 0;
...
async function handleProxy(c, protocol) {
    ...
    try {
        ...
        recordStats({ channel, source, protocol, model: exposedModel, isError: true, usage: ZERO_USAGE, costs: null });
        logSystemError('proxy', `上游调用失败 [${exposedModel}]`, { msg, protocol });
        if (settings.logIo) {
            insertLog(channel, source, protocol, exposedModel, isStream, 502, latency, reqBody, null, null, msg, false, null, pricing);
        }
    ...
    if (status >= 400) {
        ...
        recordStats({ channel, source, protocol, model: exposedModel, isError: true, usage: ZERO_USAGE, costs: null });
        logSystemWarnProxy(protocol, exposedModel, status);
        if (settings.logIo) {
    ...
    if (!isStream || !upstreamRes.body) {
        ...
        // 统计始终记录（成功调用）
        recordStats({ channel, source, protocol, model: exposedModel, isError: false, usage: norm, costs });
        if (settings.logIo) {
    ...
    collectStream(forLog, {
        channel, source, protocol, model: exposedModel, status, latency, reqBody, wantBody, logIo, pricing,
    }).catch(() => { });
    const headers = new Headers();
...
function logSystemWarnProxy(protocol, model, status) {
    logSystemWarn('proxy', `上游返回 ${status} [${model}]`, { protocol, status });
}
...
async function collectStream(stream, ctx) {
    ...
    if (ctx.logIo) {
        insertLog(ctx.channel, ctx.source, ctx.protocol, ctx.model, true, ctx.status, ctx.latency, ctx.reqBody, ctx.wantBody ? raw : null, ctx.wantBody && chunks.length ? JSON.stringify(chunks) : null, ctx.status >= 400 ? raw : null, aborted, usage, ctx.pricing);
    }
}
/** 统计 upsert 预编译语句（按日落表，独立于日志）。 */
const statsUpsertStmt = sqlite.prepare(`INSERT INTO t_proxy_daily_stats
...
function recordStats(ctx) {
    ...
    enqueueWrite(() => {
        try {
            statsUpsertStmt.run(channelId, channelName, sourceId, ctx.protocol, ctx.model || null, errInc, ctx.usage.inputTokens, ctx.usage.cachedInputTokens, ctx.usage.outputTokens, totalCost);
        }
        catch (e) {
...
function insertLog(channel, source, protocol, model, isStream, status, latency, reqBody, resBody, resChunks, error, aborted, usage = null, pricing = NULL_PRICING) {
    const norm = normalizeUsage(usage, protocol);
    const costs = computeCost(norm, pricing);
    const values = {
